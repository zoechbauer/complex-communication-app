      ...
      ${posts
        .map(post => {
          ...
          return `<a href="/post/${
            ...
            on ${postDate.toLocaleDateString()} ${postDate.toLocaleTimeString()}</span>
            </a>`;
        })
        .join('')}
    </div>`;
    } else {
      this.resultsArea.innerHTML = `<p class="alert alert-danger text-center shadow-sm">Sorry, but we could not find any results for your search</p>`;
    }
    this.hideLoaderIcon();
